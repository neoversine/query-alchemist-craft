...
const Index = () => {
  ...
  const generateQueries = async () => {
    ...
    //webhook conn
    try {
      const response = await fetch('https://n8n.finnofarms.in/webhook/dff2fee1-d41c-4066-9b09-5d9eedcdaf74', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        ...
        const errorData = await response.text(); // Use .text() if it might not be JSON
        console.error('Webhook error response:', errorData);
        throw new Error(`Failed to send data to webhook: ${response.status} ${response.statusText} - ${errorData}`);
      }

      const rawN8nResponse = await response.json(); // Expected: { output: "1. Query...\n2. Query...\n..." }
      const aiGeneratedString = rawN8nResponse.output; // Get the string generated by the AI agent

      // --- Start Parsing AI Generated String ---
      const lines = aiGeneratedString.split('\n').filter(line => line.trim() !== '');

      const parsedResults: Array<{ query: string; explanation: string; category: string }> = [];
      const categories = [
        "Community Insights",
        "Educational Content",
        "Expert Resources"
      ...
      const explanations = [
        "Search Reddit discussions for real user experiences and tips",
        "Find comprehensive guides, tutorials, and PDF resources",
        "Locate authoritative guides while excluding promotional content"
      ];

      let currentCategoryIndex = 0;
      for (let i = 0; i < lines.length; i++) {
        // Look for the "1. Final Search Query #1" pattern
        if (lines[i].startsWith(`${currentCategoryIndex + 1}. Final Search Query #`)) {
          // The actual query should be on the next line
          if (lines[i + 1]) {
            parsedResults.push({
              query: lines[i + 1].trim(), // The actual query string
              explanation: explanations[currentCategoryIndex], // Use the predefined explanation
              category: categories[currentCategoryIndex] // Use the predefined category
            });
            currentCategoryIndex++;
            i++; // Increment 'i' again to skip the query line, moving to the next potential header
          }
        }
        if (currentCategoryIndex >= 3) break; // We expect exactly 3 queries
      }

      // If less than 3 results were parsed (e.g., AI didn't follow format exactly)
      if (parsedResults.length < 3) {
          console.warn("AI did not return all 3 queries in the expected format. Falling back or handling partial results.");
          // You might want to fill in default values or show an error here
      }
